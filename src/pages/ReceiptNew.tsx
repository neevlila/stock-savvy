...
export default function ReceiptNew() {
  ...
  const addItem = () => setItems([...items, { product_id: "", expected_qty: 0, received_qty: 0 }]);
  ...
  const saveMutation = useMutation({
    mutationFn: async (status: string) => {
      ...
      const { data: receipt, error } = await supabase.from("receipts").insert({
        reference_no: "", supplier_name: supplierName, warehouse_id: warehouseId,
        status, notes, created_by: user?.id,
      }).select().single();
      if (error) throw error;

      const receiptItems = items.map(item => ({
        receipt_id: receipt.id, product_id: item.product_id,
        expected_qty: item.expected_qty, received_qty: item.received_qty,
      }));
  ...
  return (
    <div className="max-w-3xl">
              ...
              <TableBody>
                {items.map((item, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell><Input type="number" className="h-8 font-mono" value={item.expected_qty} onChange={e => updateItem(i, "expected_qty", parseInt(e.target.value) || 0)} /></TableCell>
                    <TableCell><Input type="number" className="h-8 font-mono" value={item.received_qty} onChange={e => updateItem(i, "received_qty", parseInt(e.target.value) || 0)} /></TableCell>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <Button onClick={() => saveMutation.mutate("Done")} disabled={saveMutation.isPending}>Validate & Receive</Button>
      </div>
    </div>
  );
}
